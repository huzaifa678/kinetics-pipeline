...
ROLE_GITOPS_CONTRACT_READ="$(tfout gha_gitops_contract_read_role_arn)"
[ -n "$ROLE_GITOPS_CONTRACT_READ" ] || ROLE_GITOPS_CONTRACT_READ="arn:aws:iam::${ACCOUNT_ID}:role/${NAME}-gha-gitops-contract-read"
...
gh variable set AWS_ROLE_GITOPS_CONTRACT_READ --repo "$REPO" --body "$ROLE_GITOPS_CONTRACT_READ"
